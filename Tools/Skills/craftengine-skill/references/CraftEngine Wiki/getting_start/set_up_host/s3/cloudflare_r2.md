...
### 第八步
查看跳转的页面，找到 `访问密钥 ID` 作为 `access_key_id`，找到 `机密访问密钥` 作为 `access_key_secret`。最后你应该会编写出以下配置。 \
  ...
  delivery:
    hosting:
      - type: s3
        ...
        bucket: "114514"
        access_key_id: "asodjhadosdnoad"
        access_key_secret: "asd541faf5asd5fs15asdf45as"
        protocol: "https"
        path_style: false
        region: "auto"
        upload_path: "server_resource_pack.zip"
        disable_calculate_sha256: true
        validity: 10
        rate_map:
          max_requests: 5
          reset_interval: 20
