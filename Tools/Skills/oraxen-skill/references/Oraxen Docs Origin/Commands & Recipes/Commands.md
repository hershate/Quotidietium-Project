...
![Recipe showcase using /o recipes show all](/assets/recipe_showcase.png)

#### Usage:

```yaml
/oraxen recipes builder <builder> # Creates a recipe builder of type <builder> and opens it
/oraxen recipes save <name> # Saves your recipe with name <name>
/oraxen recipes show all # Shows you the loaded recipes
/oraxen recipes show <recipe> # Shows you one recipe
```
...
This command displays a list of available emojis/glyphs. The output depends on the `emoji_list_permission_only` setting (see [Glyphs Settings](/plugin-setup/plugin-settings#glyphs)).
